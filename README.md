...
| Products | Scaffold | `go.mod` only, no Go source, no Dockerfile |
...
├── products/                # Products microservice (scaffold: go.mod only)
...
Module scaffold only. Intended to handle `CreateProduct(Product)` and `GetProduct(repeated Product)`.

- `go.mod` present (`ecommerce-products`), no Go source files, no Dockerfile, not in docker-compose.
...
Generates Go gRPC stubs from `api/proto/*.proto` into `api/gen/{auth,order,payment,stock,products}`. This is required — the generated code is not committed.
...
The project uses a Go workspace (`go.work`) covering `api`, `auth`, `gateway`, `orders`, `payments`, `products`, `shared`, and `stock`. Run `go work sync` after changing module dependencies.
